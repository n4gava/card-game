...
public class DraggableTestWithActions : MonoBehaviour {
    ...
    private DraggingActionsTest dragginAction;
    ...
    void Awake()
    {
        dragginAction = GetComponent<DraggingActionsTest>();
    }

    void OnMouseDown()
    {
        if (dragginAction == null || dragginAction.CanDrag)
        {
            dragging = true;
            dragginAction?.OnStartDrag();
    ...
    void Update ()
    {
        ...
        { 
            ...
            dragginAction?.OnDraggingInUpdate(mousePos);
