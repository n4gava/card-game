...
public class AITurnMaker: TurnMaker {
    ...
    {
        foreach (CardLogic cardLogic in p.hand.CardsInHand)
        {
            if (cardLogic.CanBePlayed)
            {
                if (cardLogic.CardAsset.MaxHealth == 0)
                {
                    // code to play a spell from hand
                    // TODO: depending on the targeting options, select a random target.
                    if (cardLogic.CardAsset.Targets == TargetingOptions.NoTarget)
                    {
                        p.PlaySpellFromHand(cardLogic, null);
                        InsertDelay(1.5f);
                ...
                else
                {
                    ...
                    p.PlayACreatureFromHand(cardLogic, 0);
                    InsertDelay(1.5f);
                    return true;
                }
        ...
        foreach (CreatureLogic cl in p.table.CreaturesOnTable)
        {
            if (cl.AttacksLeftThisTurn > 0)
            {
                ...
                if (p.OtherPlayer.table.CreaturesOnTable.Count > 0)
                {
                    ...
                    cl.Attack(targetCreature);
                }                    
                else
                    cl.Attack(p.OtherPlayer);
